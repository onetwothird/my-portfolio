...
export default function LiveVisitorCount() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-end gap-2 animate-pulse mb-6">
    ...
    <div className="flex flex-col items-end gap-3 mb-6">
      ...
      <div className="flex items-center -space-x-3">
        {avatarSeeds.map((seed, index) => (
          <div 
            key={seed} 
            className="w-10 h-10 rounded-full border-2 border-white/20 bg-white overflow-hidden relative"
        ...
        {remainingCount > 0 && (
          <div 
            className="w-auto min-w-10 px-2 h-10 rounded-full border-2 border-white/20 bg-white/10 backdrop-blur-md flex items-center justify-center text-xs font-medium text-white relative shadow-sm"
            style={{ zIndex: 0 }}
          >
            +{remainingCount}
          </div>
      ...
      <div className="flex flex-col items-end text-right">
         <div className="text-sm font-medium text-white/90 drop-shadow-sm">
           <span className="font-bold text-white text-base mr-1">{stats.activeNow}</span> 
           people viewing now
         </div>
         <div className="text-xs font-medium text-white/60 mt-0.5">
